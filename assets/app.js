(function () {
        ...
        <section class="kpi-grid">
          ...
          ${kpiCard(
            ...
            [
              ...
              { label: "Invoices synced", value: synced, caption: "Live in Zoho Books" },
    ...
    return `
      <article class="order-card">
        <div>
          ...
          <p class="muted">${h.customerName(order.customerId)} / <span class="mono">${order.id}</span></p>
          ${order.inquiryId && order.inquiryId.startsWith("inq") ? `<p class="card-note muted">From sales inquiry <span class="mono">${order.inquiryId}</span></p>` : ""}
        </div>
  ...
  function bindOrderBoard() {
    document.querySelectorAll("[data-order-stage]").forEach((select) => {
      select.addEventListener("change", (event) => {
        const orderId = event.target.dataset.orderStage;
        const order = state.orders.find((item) => item.id === orderId);
        state.orderStages[orderId] = event.target.value;
        if (order) {
          order.stage = event.target.value;
          order.completionPct = STAGE_PROGRESS[event.target.value] || order.completionPct;
          state.activeOrderId = order.id;
          if (event.target.value === "Invoiced") {
            markOrderInvoiceReady(order);
          }
          addActivity("Operations", `Moved ${order.id} to ${event.target.value}.`);
        }
        saveState();
        renderCurrentRoute();
      });
  ...
  function renderAccounting() {
    return `
      <div class="stack">
        ${workflowGuide("invoice", "Sync ready invoices to Zoho Books and track receivables.")}
        <section class="table-card invoice-table">
          <div class="table-header">
            <div>
              <h2>Invoice Readiness &amp; Receivables</h2>
              <p>One row per order — documents, payment holds, and Zoho Books sync.</p>
            </div>
  ...
  function bindAccounting() {
    document.querySelectorAll("[data-sync-doc]").forEach((button) => {
      button.addEventListener("click", (event) => {
        ...
        if (!doc) return;
        doc.syncStatus = "Synced to Zoho Books";
        ...
        addActivity("Accounts", `Synced ${doc.id} to Zoho Books and marked payment follow-up.`);
        saveState();
        renderCurrentRoute();
      });
    });
  ...
  function bindSales() {
    ...
    document.querySelectorAll("[data-inquiry-status]").forEach((select) => {
      select.addEventListener("change", (event) => {
        const inquiry = state.inquiries.find((item) => item.id === event.currentTarget.dataset.inquiryStatus);
        if (!inquiry) return;
        inquiry.status = event.currentTarget.value;
        inquiry.nextAction =
          event.currentTarget.value === "Won"
            ? "Convert to production order"
            : event.currentTarget.value === "Lost"
              ? "Record reason and nurture later"
              : "Follow up with customer";
        addActivity("Sales", `Moved ${inquiry.id} to ${inquiry.status}.`);
        saveState();
        renderCurrentRoute();
      });
    });
