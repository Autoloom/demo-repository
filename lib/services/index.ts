...
import { productionGateBlockers, dispatchGateBlockers, latestInspection, normalizeInquiry, documentTray, type InquiryBoardItem } from "@/lib/domain/pipeline";
...
export type { DocumentState, InquiryBoardItem } from "@/lib/domain/pipeline";
...
/** Normalize legacy enquiries at every service read; the next mutation persists migration. */
async function readStore(): Promise<CableStore> {
  const store = await adapter.read();
  store.inquiries = store.inquiries.map(normalizeInquiry);
  return store;
}
...
export const inquiriesService = {
  list: async () => (await readStore()).inquiries,
  get: async (inquiryId: string) => (await readStore()).inquiries.find((item) => item.id === inquiryId) ?? null,
  board: async (): Promise<InquiryBoardItem[]> => {
    const store = await readStore();
    return store.inquiries.map((inquiry) => ({ ...inquiry, documents: documentTray(store, inquiry.id) }));
  },
  create: (input: Pick<Inquiry, "customerId" | "requirement" | "specIds" | "source" | "estimatedValueInr" | "followUpDate" | "commercialTerms">, actor: Actor) =>
    mutateStore((store) => {
      if (!can(actor.role, "create", "inquiry")) throw new Error("You cannot create enquiries.");
      if (!store.customers.some((customer) => customer.id === input.customerId)) throw new Error("Choose an existing customer.");
      if (input.specIds.some((specId) => !store.specs.some((spec) => spec.id === specId))) throw new Error("Choose existing cable specifications.");
      if (input.requirement.trim().length < 12 || !Number.isFinite(input.estimatedValueInr) || input.estimatedValueInr <= 0 || !/^\d{4}-\d{2}-\d{2}$/.test(input.followUpDate)) throw new Error("Enter a requirement, positive value and follow-up date.");
      let sequence = store.inquiries.length;
      while (store.inquiries.some((inquiry) => inquiry.id === id("INQ", sequence))) sequence += 1;
      const inquiry: Inquiry = { ...input, specIds: [...new Set(input.specIds)], requirement: input.requirement.trim(), id: id("INQ", sequence), stage: "New", ownerRole: actor.role, nextAction: "Qualify customer requirement", createdAt: isoNow() };
      store.inquiries.unshift(inquiry);
      addActivity(store, { actorRole: actor.role, actorName: actor.name, type: "inquiry.created", recordType: "inquiry", recordId: inquiry.id, text: `${inquiry.id} created with ${inquiry.specIds.length} cable specifications.` });
      return inquiry;
    }),
  updateDetails: (inquiryId: string, input: Pick<Inquiry, "customerId" | "requirement" | "specIds" | "source" | "estimatedValueInr" | "followUpDate" | "commercialTerms">, actor: Actor) =>
    mutateStore((store) => {
      if (!can(actor.role, "edit", "inquiry")) throw new Error("You cannot edit enquiries.");
      const inquiry = store.inquiries.find((item) => item.id === inquiryId);
      if (!inquiry) throw new Error("Inquiry not found");
      if (!store.customers.some((customer) => customer.id === input.customerId)) throw new Error("Choose an existing customer.");
      if (input.customerId !== inquiry.customerId && store.quotes.some((quote) => quote.inquiryId === inquiryId)) throw new Error("The customer cannot change after a quote is linked.");
      if (input.specIds.some((specId) => !store.specs.some((spec) => spec.id === specId))) throw new Error("Choose existing cable specifications.");
      if (input.requirement.trim().length < 12 || !Number.isFinite(input.estimatedValueInr) || input.estimatedValueInr <= 0 || !/^\d{4}-\d{2}-\d{2}$/.test(input.followUpDate)) throw new Error("Enter a requirement, positive value and follow-up date.");
      Object.assign(inquiry, input, { specIds: [...new Set(input.specIds)], requirement: input.requirement.trim() });
      addActivity(store, { actorRole: actor.role, actorName: actor.name, type: "inquiry.updated", recordType: "inquiry", recordId: inquiryId, text: `${inquiryId} cable selection and commercial details updated.` });
      return inquiry;
    }),
  updateTitle: (inquiryId: string, title: string, actor: Actor) =>
    mutateStore((store) => {
      if (!can(actor.role, "edit", "inquiry")) throw new Error("You cannot edit enquiries.");
      const inquiry = store.inquiries.find((item) => item.id === inquiryId);
      if (!inquiry) throw new Error("Inquiry not found");
      if (!title.trim()) throw new Error("A requirement is required.");
      inquiry.requirement = title.trim();
      addActivity(store, { actorRole: actor.role, actorName: actor.name, type: "inquiry.title_updated", recordType: "inquiry", recordId: inquiryId, text: `${inquiryId} requirement updated.` });
      return inquiry;
    }),
  updateStage: (inquiryId: string, stage: Inquiry["stage"], actor: Actor) =>
    mutateStore((store) => {
      const inquiry = store.inquiries.find((item) => item.id === inquiryId);
      if (!inquiry) throw new Error("Inquiry not found");
      if (!can(actor.role, "transition", "inquiry")) throw new Error("You cannot move enquiries.");
      inquiry.stage = stage;
      inquiry.nextAction = stage === "Won" ? (inquiry.convertedOrderId ? "Converted to order" : "Send an approved quote to the order board") : inquiry.nextAction;
      addActivity(store, {
        actorRole: actor.role,
        actorName: actor.name,
...
export const ordersService = {
  ...
  gateSummaries: async () => {
    const store = await readStore();
    return Object.fromEntries(store.orders.map((order) => [order.id, {
      production: productionGateBlockers(store, order), dispatch: dispatchGateBlockers(store, order),
      documents: documentTray(store, order.inquiryId, order.id),
    }]));
  },
  /**
...
export const inspectionService = {
  ...
  logReport: (input: Omit<InspectionReport, "id">, actor: Actor) =>
    mutateStore((store) => {
      if (!store.orders.some((order) => order.id === input.orderId)) throw new Error("Order not found");
      if (!Number.isFinite(Date.parse(input.inspectedAt))) throw new Error("Enter a valid inspection date.");
      ...
      if (order) {
        order.inspectionStatus = latestInspection(store.inspectionReports, order.id)?.result;
      }
      const linkedDispatch = store.dispatches.find((entry) => entry.orderId === report.orderId);
      const clearanceItem = linkedDispatch?.checklist.find((entry) => entry.id === "clearance");
      if (clearanceItem && order) clearanceItem.done = dispatchGateBlockers(store, order).length === 0;
      if (order?.stage === "Ready for Dispatch" && !linkedDispatch?.dispatchedAt && dispatchGateBlockers(store, order).length) {
        order.stage = "In Production";
        order.completionPct = 60;
        const invoice = store.invoices.find((item) => item.orderId === order.id);
        if (invoice && invoice.syncStatus !== "Synced to Zoho Books") {
          invoice.syncStatus = "Missing dispatch data";
          if (invoice.status === "Ready to sync") invoice.status = "Blocked";
        }
        addActivity(store, { actorRole: actor.role, actorName: actor.name, type: "order.dispatch_clearance_revoked", recordType: "order", recordId: order.id, text: `${order.id} returned to production — latest inspection has no dispatch clearance.` });
      }
      if (report.result === "Passed" && report.clearanceIssued) {
        ...
        if (clearance && order) clearance.done = dispatchGateBlockers(store, order).length === 0;
...
function completeDispatchHandoff(store: CableStore, dispatch: Dispatch, actor: Actor) {
  ...
  // A checklist update/report must still persist when blocked; only handoff is skipped.
  if (!order || dispatchGateBlockers(store, order).length > 0) return;
...
export const invoicesService = {
  ...
  sync: (invoiceId: string, actor: Actor) =>
    mutateStore((store) => {
      ...
      if (order) {
        const blockers = dispatchGateBlockers(store, order);
        if (blockers.length) throw new Error(`Dispatch is gated for ${order.id}: ${blockers.join(" ")}`);
      }
...
export const settingsService = {
  getOrg: async () => (await readStore()).org,
  getPolicies: async () => (await readStore()).policies,
...
export const journeyService = {
  get: async (recordId: string) => {
    ...
    const quote = order ? store.quotes.find((item) => item.id === order.quoteId) : store.quotes.find((item) => item.id === recordId || item.inquiryId === recordId);
    const inquiry = quote?.inquiryId
      ? store.inquiries.find((item) => item.id === quote.inquiryId)
      : order?.inquiryId
    ...
    return { customer, inquiry, quote, order, dispatch, invoice, documents: documentTray(store, inquiry?.id, order?.id) };
  },
};
