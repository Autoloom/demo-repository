...
test("a tolerance survives being persisted onto a GTP record", () => {
  // `derivedFields: fields` assigns a ResolvedField[] VARIABLE into a GtpDerivedField[] slot.
  // Excess-property checking only fires on fresh object literals, so if GtpDerivedField were
  // missing the `tolerance` member this would compile clean and silently drop every tolerance
  // from every saved record. A type-level assertion cannot catch that; a round trip can.
  const fields = ab();
