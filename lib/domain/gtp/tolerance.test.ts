...
import { bandTolerance, floorTolerance, manualTolerance, notApplicable, withMandatoryTolerance } from "./tolerance";
...
test("a hand-typed tolerance never claims a standard", () => {
  // A custom parameter has no clause behind it. Labelling this "customer" would repeat the error
  // fixed for VALUES: an operator transcribing a tender is not the board mandating it, and
  // nothing on the row records which happened.
  const tol = manualTolerance("±2%");
  assert.equal(tol.origin, "manual");
  assert.equal(tol.value, "±2%");
  assert.doesNotMatch(tol.trace, /§/, "a hand-typed tolerance must not cite a clause");
  assert.match(tol.trace, /not derived from any standard/);
});

test("a blank tolerance is an explicit N/A, distinguishable from the engine default", () => {
  // Both print "N/A", but the trail must say which happened: a person left it unstated, or the
  // engine had nothing to say about a derived field.
  const byPerson = manualTolerance("");
  const byEngine = notApplicable("No tolerance applies to this parameter");

  assert.equal(byPerson.value, "N/A");
  assert.equal(byPerson.origin, "not-applicable");
  assert.match(byPerson.trace, /hand-added/);
  assert.notEqual(byPerson.trace, byEngine.trace, "the two N/A reasons must not be identical");
});

test("whitespace is not a stated tolerance", () => {
  for (const blank of ["", "   ", "\t", "\n  "]) {
    assert.equal(manualTolerance(blank).value, "N/A", `"${blank}" should not count as stated`);
    assert.equal(manualTolerance(blank).origin, "not-applicable");
  }
});

test("a typed tolerance is stored exactly as written", () => {
  // Free text on purpose: a buyer's clause may state a band, a class, or a bare limit. The sign
  // convention that governs DERIVED tolerances does not apply — normalising here would invent a
  // meaning the operator did not write.
  for (const typed of ["±2%", "−5%", "Class B", "+0/-0.5 mm", "as per IS 8130"]) {
    assert.equal(manualTolerance(typed).value, typed, `"${typed}" was altered`);
    assert.equal(manualTolerance(typed).origin, "manual");
  }
  // Surrounding whitespace is trimmed, but the value itself is untouched.
  assert.equal(manualTolerance("  ±2%  ").value, "±2%");
});

test("withMandatoryTolerance leaves a hand-typed tolerance alone", () => {
  // The custom-parameter path sets its tolerance before the field reaches any engine boundary.
  // If withMandatoryTolerance overwrote it, a typed "±2%" would silently become N/A.
  const field = {
    key: "custom.1", label: "Tender clause 4.2", value: "Confirmed",
    tag: "MANUAL" as const, source: "override" as const,
    trace: "Added manually — not derived from any standard", editable: true,
    tolerance: manualTolerance("±2%"),
  };
  const [out] = withMandatoryTolerance([field]);
  assert.equal(out.tolerance?.value, "±2%");
  assert.equal(out.tolerance?.origin, "manual");
});
