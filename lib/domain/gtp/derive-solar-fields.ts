...
export function deriveSolarFields(
  ...
  // ── Finished mass ─────────────────────────────────────────────────────────────────────────
  // The tables give a mean overall diameter but no conductor diameter, so the conductor's
  // circumscribed circle is back-derived from it. That circle is much larger than the nominal
  // cross-section implies — these are Class 5 conductors, hundreds of fine strands with air
  // between them — which is exactly why the CONDUCTOR mass uses the nominal CSA and only the
  // insulation annulus uses the derived diameter.
  const conductorDiaMm =
    dims.values.meanOverallDiaMm -
    2 * (dims.values.insulationThicknessMm + dims.values.sheathThicknessMm);
  const massResult = deriveSolarMass({
    csaSqMm: config.csaSqMm,
  ...
  if (isMassGap(massResult)) {
    ...
    fields.push({
      key: "fin.totalMass",
      label: "Total mass (approx.)",
      value: `${massResult.totalKgPerKm.toFixed(1)} kg/km`,
      tag: "CALC",
      source: "calc",
      ...
      tolerance: bandTolerance(
        "±5%",
        "works-estimate",
        "Works spread on a mass derived from the IS 17293 dimensional table — see the trace for each layer",
      ),
    });
  }
