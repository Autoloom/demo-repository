...
test("only genuinely universal fields are shared between types", () => {
  ...
  // Manufacturer identity and rated voltage are the same on any LT cable.
  //
  // `fin.totalMass` joined this list deliberately. Mass is the one DERIVED quantity every cable
  // type has, and the quote prices it — so it carries one key across all types, letting the
  // costing read it without per-type branching. That is the opposite of the leak this test
  // guards against: the point is that a caller must NOT have to know which engine produced it.
...
test("every cable type reports its mass under one key, so the quote needs no branching", () => {
  // WP-1's whole purpose. Before this, only AB derived a mass; LT and solar emitted nothing, so
  // wiring the quote to the engine would have silently regressed them to a zero price.
  for (const [name, fields] of [["AB", ab()], ["LT", power()]] as const) {
    const m = fields.find((f) => f.key === "fin.totalMass");
    assert.ok(m, `${name} does not report a mass`);
    assert.match(String(m?.value), /kg\/km/, `${name} mass is not in kg/km`);
    assert.ok(m?.tolerance, `${name} mass carries no tolerance`);
  }
});
