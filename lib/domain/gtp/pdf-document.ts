...
/**
 * Map a live field's tag to the coarser provenance a STORED GTP row carries.
 *
 * Lives here rather than in the builder because logic in a component is untestable under this
 * repo's `lib/**` test glob, and this particular mapping is a provenance claim — the kind of
 * thing that should fail a test rather than a code review.
 *
 * The three outcomes, and why:
 *   • LOOKUP / CALC  → is-standard.  Read from an encoded table, or computed from one.
 *   • QUIRK          → client-fixed. A genuine customer requirement, which is what that label
 *                      means to the reviewer reading it.
 *   • MANUAL         → manual.       A person typed it and nothing verifies it. This used to
 *                      collapse into client-fixed, asserting the buyer required something an
 *                      operator invented.
 *   • CHOICE / FIXED → is-standard.  An option the standard offers, or an org constant. Ours to
 *                      stand behind, and not a customer mandate.
 */
export function sectionSourceFor(tag: FieldTag): GtpSectionSource {
  switch (tag) {
    case "MANUAL":
      return "manual";
    case "QUIRK":
